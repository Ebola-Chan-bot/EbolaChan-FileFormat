...
namespace 埃博拉酱文件格式
{
	struct 句柄式分配器
	{
		...
		//构造句柄式分配器，要求你提供一个pmr::vector<char>作为底层的资源分配器，文件的全部字节将存放在此向量中。
		constexpr 句柄式分配器(std::pmr::vector<char>& 底层资源) :底层资源(底层资源) {}
		//分配一个指定字节数的内存块，返回块索引。块索引可以用于创建句柄，也可以直接用分配器和块索引访问内存指针。
		uint64_t 分配(uint64_t 字节数);
		//改变块索引指向的内存块大小。如果新大小小于原大小，原数据将被截断。如果新大小大于原大小，新数据是未定义的。可以对未分配的块索引调用此方法，例如在新文件中为预定义的固定索引块分配空间。
		void 重分配(uint64_t 块索引, uint64_t 字节数);
		//返回索引指向的内存块的指针。该指针只能临时使用，对分配器和底层资源的任何操作都可能使指针失效。
		void* 取指针(uint64_t 块索引);
		//释放块索引指向的内存，以允许被其它块占用。内存块中的数据将丢失，索引可能被回收再次分配。可以使用重分配方法使得该索引再次可用。
		void 释放(uint64_t 块索引);
		//在指定块索引处构造一个对象。该索引必须已分配足够大的内存块。
		template<typename 构造类型,typename...参数类型>
		void 构造(uint64_t 块索引, 参数类型&&...参数)
		{
			new(取指针(块索引)) 构造类型(std::forward<参数类型>(参数)...);
		}
	};
	using 内存句柄 = uint64_t;
